   ...
   * */
	...
	static async findAll(filters = {}) {
		...
		if (minEmployees > maxEmployees) {
			throw new BadRequestError('Please enter new search, minEmployees cannot be greater than maxEmployees');
		}
		...
		if (name) {
			...
			filterExp.push(`name ILIKE $${searchValues.length}`);
		}

		if (minEmployees !== undefined) {
			searchValues.push(minEmployees);
			filterExp.push(`num_employees >= $${searchValues.length}`);
		}
		...
		if (filterExp.length > 0) {
			companiesRes += ' WHERE ' + filterExp.join(' AND ');
		}
		...
		const filteredCompaniesRes = await db.query(companiesRes, searchValues);
