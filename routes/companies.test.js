...
test('filtering with 1 query', async function() {
	const resp = await request(app).get('/companies').query({ name: '3' });
	expect(resp.body).toEqual({
		companies: [
			{
				handle: 'c3',
...
test('Error when using non-existent filter', async function() {
	const resp = await request(app).get('/companies').query({ wrong: 'error' });
	expect(resp.statusCode).toEqual(400);
});
	...
	test('works for anon', async function() {
		...
		expect(resp.body).toEqual({
			company: {
				...
				logoUrl: 'http://c1.img',
				jobs: [
					{ id: testJobs[0], title: 'J1', equity: '0.1', salary: 10 },
					{ id: testJobs[1], title: 'J2', equity: '0.2', salary: 25 },
					{ id: testJobs[2], title: 'J3', equity: null, salary: 50 }
				]
			}
		});
	});
