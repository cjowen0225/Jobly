...
describe('POST /users', function() {
	...
	test('Unautorized for Non-Logged in User', async function() {
	...
	test('Unauthorized for Incorrect User', async function() {
		const resp = await request(app)
			.post('/users')
			.send({
				username: 'u-new',
...
describe('POST /users/:username/jobs/:id', function() {
	test('works for admin', async function() {
		const resp = await request(app)
			.post(`/users/u1/jobs/${testJobs[1]}`)
			.set('authorization', `Bearer ${adminToken}`);
		expect(resp.body).toEqual({ applied: testJobs[1] });
	});

	test('works for correct user', async function() {
		const resp = await request(app).post(`/users/u1/jobs/${testJobs[1]}`).set('authorization', `Bearer ${u1Token}`);
		expect(resp.body).toEqual({ applied: testJobs[1] });
	});

	test('unauth for incorrect user', async function() {
		const resp = await request(app).post(`/users/u1/jobs/${testJobs[1]}`).set('authorization', `Bearer ${u2Token}`);
		expect(resp.statusCode).toEqual(401);
	});

	test('unauth for anon', async function() {
		const resp = await request(app).post(`/users/u1/jobs/${testJobs[1]}`);
		expect(resp.statusCode).toEqual(401);
	});

	test('not found if no such user', async function() {
		const resp = await request(app)
			.post(`/users/nope/jobs/${testJobs[1]}`)
			.set('authorization', `Bearer ${adminToken}`);
		expect(resp.statusCode).toEqual(404);
	});
